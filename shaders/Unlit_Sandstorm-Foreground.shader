// Rewrite of the game's "Unlit/Sandstorm-Foreground" for GLES3.
// _SandTex is sampled twice in world space (scales _SandTex_ST.x and .y), scrolled by
// _SandTex_ST.zw * time, wobbled by _DistortTex and snapped to the _WorldToPixel grid.
// Sand density picks lerp(_Color2, _Color1, d*d); alpha = pow(d, _MaskPow) * _MaskMultiplier
// times the _MainTex alpha (mesh uv). Alpha blended, no depth write.
Shader "Unlit/Sandstorm-Foreground" {
	Properties {
	...
	SubShader {
		...
		Pass {
			...
			float _OpacityMul, _SandBoost, _DistortStrength, _WorldToPixel, _MaskPow, _MaskMultiplier;
			...
			struct v2f {
				...
				float4 uv : TEXCOORD1;      // xy: _DistortTex (world), zw: _MainTex (mesh uv)
			};
			...
			fixed4 frag (v2f i) : SV_Target {
				...
				float alpha = saturate(pow(sand, _MaskPow) * _MaskMultiplier);
				fixed4 c;
				c.rgb = lerp(_Color2.rgb, _Color1.rgb, sand * sand);
				c.a = alpha * tex2D(_MainTex, i.uv.zw).a;
				return c;
			}
			ENDCG
		}
