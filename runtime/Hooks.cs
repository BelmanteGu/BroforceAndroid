    ...
    public static class Hooks
    {
        ...
        public static void OnStartup()
        {
            if (Application.platform == RuntimePlatform.Android)
            {
                ...
                try { StartOverlay(); }
                catch (System.Exception e) { Debug.Log("[BroforceAndroid] overlay: " + e.Message); }
            }
        ...
        /// <summary>
        /// Starts the native overlay (android/src): on-screen controls, the quick settings
        /// panel and the screen mode (edge to edge under the camera cutout by default).
        /// </summary>
        static void StartOverlay()
        {
            using (var player = new AndroidJavaClass("com.unity3d.player.UnityPlayer"))
            using (var activity = player.GetStatic<AndroidJavaObject>("currentActivity"))
            using (var overlay = new AndroidJavaClass("io.github.belmantegu.broforceandroid.Overlay"))
                overlay.CallStatic("attach", activity);
        }
